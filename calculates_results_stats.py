...
#       with this function
#
def calculates_results_stats(results_dic: dict) -> dict:
    """
    ...
    for k, v in results_dic.items():
        ...
        results_stats_dic["n_correct_dogs"] += int(v[3] and v[4])

        # B: Number of Dog Images
        results_stats_dic["n_dogs_img"] += int(v[3])

        # C: Number of Correct Non-Dog matches
        results_stats_dic["n_correct_notdogs"] += not v[3] and not v[4]

        # D: Number of Not Dog Images
        results_stats_dic["n_notdogs_img"] += int(not v[3])

        # E: Number of Correct Breed matches
        results_stats_dic["n_correct_breed"] += int(v[3] and v[2])

        # Y: Number of label matches
        results_stats_dic["n_match"] += int(v[2])
